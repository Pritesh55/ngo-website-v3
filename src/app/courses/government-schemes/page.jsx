                ...
                <div className="space-y-2">
                  {swavlambanDocs.map((doc, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-5 h-5 rounded-md flex-shrink-0 mt-0.5 border flex items-center justify-center transition-colors ${isChecked ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-350 bg-white'
                          }`}>
                          {isChecked && <Check className="w-3.5 h-3.5 stroke-[3]" />}
                        </div>
                        <span className={`text-sm md:text-base leading-relaxed ${isChecked ? 'line-through text-slate-500' : 'font-medium'}`}>
                          ({idx + 1}) {doc}
                        </span>
                      </div>
                    )
